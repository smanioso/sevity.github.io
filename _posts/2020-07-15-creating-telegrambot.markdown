...
뭐 API 문서를 보면 이런저런 파라미터가 필요하지만 지금은 그냥 파라미터 없이 한번 호출을 해보았습니다. 브라우저에 그냥 입력하시면 되고요.
결과는 아래와 같이 나오네요.
...
텔레그램으로 모니터링하는건 상당히 유용하고 저도 자주 사용합니다만 주의해야할 점들이 몇가지 있습니다.

1. 텔레그램 전송이 그렇게 빠르지 않음. 즉 성능 저하를 일으킬수 있습니다.
2. 텔레그램 전송이 실패하거나 하는 경우가 왕왕 있음
3. 코드 실행자체가 안되는 경우는 모니터링이 안되는 경우가 있음

위의 예제에서는 이러한 문제들이 그대로 있으니 이러한 점을 생각하셔서 적용을 하시면 좋을 것 같습니다.
